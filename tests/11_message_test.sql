...
-- Simulation de l'appartenance au groupe dans la table de liaison de ton système (ex: group_members)
...
SELECT lives_ok(
    $$ INSERT INTO messages (conversation_id, owner_id, content) VALUES (801, 500, 'Bonjour l''équipe !') $$,
...
SELECT ok(
    (SELECT count(*) FROM unread_counters WHERE conversation_id = 802 AND user_id = 501) = 0,
    'La ligne dans unread_counters doit avoir été supprimée automatiquement'
);
